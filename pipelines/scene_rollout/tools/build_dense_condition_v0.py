...
def build_player_metadata(episode_dir: Path, ego_stem: str) -> dict[str, Any]:
    ...
    by_handle = {
        str(p["entity_handle"]): p
        for p in players
        if p.get("entity_handle") not in (None, "", "None")
    }
    by_player_index = {int(p["player_index"]): p for p in players}
    ego = next(
        p for p in players
...
def build_masks(
    ...
    for player_id, entry in vis.items():
        ...
        if not any(r["range"][0] <= frame_index <= r["range"][1] for r in ranges):
            continue
        raw_handle = entry.get("entity_handle")
        handle = str(raw_handle) if raw_handle not in (None, "", "None") else None
        player = meta["by_handle"].get(handle) if handle is not None else None
        if player is None:
            try:
                player = meta["by_player_index"].get(int(player_id))
            except (TypeError, ValueError):
                player = None
        if handle is None and player is not None and player.get("entity_handle") not in (None, "", "None"):
            handle = str(player["entity_handle"])

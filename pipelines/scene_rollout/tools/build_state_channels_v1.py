#!/usr/bin/env python3
"""Build player-state condition cache v1 for Memory dense adapter training.

Outputs one compressed npz per aligned cache row plus a jsonl manifest keyed by
clip_id. Ego alive/health are stored as [21] scalars; opponent dead-marker is a
[21,60,104] uint8 mask and is expanded by the state trainer on demand.
"""
from __future__ import annotations

import argparse
from collections import Counter, defaultdict
from datetime import datetime, timezone
import hashlib
import json
from pathlib import Path
import sys
from typing import Any, Iterable

import numpy as np

ROOT = Path(__file__).resolve().parents[1]
if str(ROOT / "tools") not in sys.path:
    sys.path.insert(0, str(ROOT / "tools"))

from interaction_channels_v0 import WEAPON_VOCABULARY, build_interaction_channels

from build_mesh_dense_condition_v0 import (  # type: ignore
    CAMERA_PITCH_OFFSET,
    CAMERA_YAW_OFFSET,
    PLAYER_HEIGHT,
    PLAYER_RADIUS,
    PLAYER_RADIUS_SCALE,
    PLAYER_SCREEN_Y_OFFSET_PX,
    PLAYER_Z_OFFSET,
    import_tool,
    parse_team_player,
    stamp_ellipse,
)

DEFAULT_CACHE_MANIFESTS = [
    Path("output/memory_v2_canonical_cleanup_20260628_v0/pilot1_aligned_cache_K11T2_motion_q50_v2only_v0.jsonl"),
    Path("output/memory_v2_canonical_cleanup_20260628_v0/pilot3_batch1_aligned_cache_K11T2_motion_q50_v2only_v0.jsonl"),
    Path("output/memory_v2_canonical_cleanup_20260628_v0/pilot3_batch2_aligned_cache_K11T2_motion_q50_v2only_v0.jsonl"),
]

EVENT_TARGET_FIELDS = (
    ("fire", "ego_fire_event", "ego_fire_weapon_onehot"),
    ("reload", "ego_reload_event", "ego_reload_weapon_onehot"),
    ("weapon_switch", "ego_weapon_switch_event", "ego_switch_target_onehot"),
    ("throw", "ego_throw_event", "ego_throw_item_onehot"),
)
ONEHOT_FIELDS = (
    "ego_current_weapon_onehot",
    "ego_fire_weapon_onehot",
    "ego_reload_weapon_onehot",
    "ego_switch_target_onehot",
    "ego_throw_item_onehot",
)


def iter_jsonl(path: Path) -> Iterable[dict[str, Any]]:
    with path.open("r", encoding="utf-8") as f:
        for line in f:
            if line.strip():
                yield json.loads(line)


def read_json(path: Path) -> Any:
    with path.open("r", encoding="utf-8") as f:
        return json.load(f)


_JSON_CACHE: dict[str, Any] = {}
_EPISODE_PLAYER_JSONS_CACHE: dict[str, list[Path]] = {}


def read_json_cached(path: Path) -> Any:
    key = str(path)
    if key not in _JSON_CACHE:
        _JSON_CACHE[key] = read_json(path)
    return _JSON_CACHE[key]


def sha1_text(text: str) -> str:
    return hashlib.sha1(text.encode("utf-8")).hexdigest()


def mtime_utc(path: Path) -> str:
    return datetime.fromtimestamp(path.stat().st_mtime, tz=timezone.utc).isoformat().replace("+00:00", "Z")


def validate_interaction_channels(
    interaction: dict[str, np.ndarray], *, clip_id: str, frame_count: int
) -> dict[str, np.ndarray]:
    event_fields = tuple(event_field for _, event_field, _ in EVENT_TARGET_FIELDS)
    required = event_fields + ONEHOT_FIELDS + ("ego_weapon_slot_name",)
    missing = [key for key in required if key not in interaction]
    if missing:
        raise ValueError(f"clip_id={clip_id}: missing interaction fields: {missing}")

    vocab_size = len(WEAPON_VOCABULARY)
    validated: dict[str, np.ndarray] = {}
    for key in event_fields:
        value = np.asarray(interaction[key])
        if value.shape != (frame_count,):
            raise ValueError(
                f"clip_id={clip_id}: {key} shape={list(value.shape)}, "
                f"expected=[{frame_count}]"
            )
        validated[key] = value.astype(np.float32, copy=False)

    for key in ONEHOT_FIELDS:
        value = np.asarray(interaction[key])
        expected = (frame_count, vocab_size)
        if value.shape != expected:
            raise ValueError(
                f"clip_id={clip_id}: {key} shape={list(value.shape)}, "
                f"expected=[{frame_count},{vocab_size}]"
            )
        validated[key] = value.astype(np.float32, copy=False)

    weapon_names = np.asarray(interaction["ego_weapon_slot_name"])
    if weapon_names.shape != (frame_count,):
        raise ValueError(
            f"clip_id={clip_id}: ego_weapon_slot_name shape={list(weapon_names.shape)}, "
            f"expected=[{frame_count}]"
        )
    if weapon_names.dtype.kind != "U":
        raise TypeError(
            f"clip_id={clip_id}: ego_weapon_slot_name dtype={weapon_names.dtype}, "
            "expected Unicode dtype"
        )
    validated["ego_weapon_slot_name"] = weapon_names
    return validated


def interaction_stats(interaction: dict[str, np.ndarray]) -> dict[str, Any]:
    vocabulary = list(WEAPON_VOCABULARY)
    event_counts: dict[str, int] = {}
    target_distribution: dict[str, dict[str, int]] = {}
    unknown_other: dict[str, dict[str, float | int]] = {}

    fields_with_masks: list[tuple[str, np.ndarray | None]] = [
        ("ego_current_weapon_onehot", None)
    ]
    for event_name, event_field, target_field in EVENT_TARGET_FIELDS:
        active = interaction[event_field] > 0.0
        event_counts[event_name] = int(active.sum())
        distribution = Counter()
        targets = interaction[target_field]
        for row in targets[active]:
            if not np.any(row > 0.0):
                distribution["missing_target"] += 1
            else:
                distribution[vocabulary[int(np.argmax(row))]] += 1
        target_distribution[event_name] = dict(sorted(distribution.items()))
        fields_with_masks.append((target_field, active))

    for field, active in fields_with_masks:
        rows = interaction[field] if active is None else interaction[field][active]
        sample_count = int(len(rows))
        unknown_count = 0
        other_count = 0
        for row in rows:
            if not np.any(row > 0.0):
                continue
            target = vocabulary[int(np.argmax(row))]
            unknown_count += int(target == "unknown")
            other_count += int(target == "other")
        unknown_other[field] = {
            "sample_count": sample_count,
            "unknown_count": unknown_count,
            "unknown_ratio": float(unknown_count / sample_count) if sample_count else 0.0,
            "other_count": other_count,
            "other_ratio": float(other_count / sample_count) if sample_count else 0.0,
        }

    return {
        "event_counts": event_counts,
        "event_target_distribution": target_distribution,
        "unknown_other": unknown_other,
    }


def finite_player(frame: dict[str, Any]) -> bool:
    try:
        return bool(np.isfinite(float(frame["x"])) and np.isfinite(float(frame["y"])) and np.isfinite(float(frame["z"])))
    except Exception:
        return False


def ego_state(frames: list[dict[str, Any]], raw_frames: list[int]) -> tuple[np.ndarray, np.ndarray, list[dict[str, Any]]]:
    alive = np.ones((len(raw_frames),), dtype=np.float32)
    health = np.ones((len(raw_frames),), dtype=np.float32)
    debug: list[dict[str, Any]] = []
    for i, frame_index in enumerate(raw_frames):
        if frame_index < 0 or frame_index >= len(frames):
            h = 100.0
            missing = True
        else:
            h = float(frames[frame_index].get("health", 100.0))
            missing = "health" not in frames[frame_index]
        alive[i] = 1.0 if h > 0.0 else 0.0
        health[i] = max(0.0, min(1.0, h / 100.0))
        debug.append({"frame": int(frame_index), "raw_health": float(h), "alive": float(alive[i]), "health_norm": float(health[i]), "fallback_health": bool(missing)})
    return alive, health, debug


def episode_player_jsons(action_json: Path) -> list[Path]:
    episode_dir = action_json.parent
    return sorted(p for p in episode_dir.glob("*.json") if p.name.startswith("Ep_") and not p.name.endswith("_episode_info.json") and not p.name.endswith("_video_manifest.json") and not p.name.endswith("_player_visibility.json"))


def episode_player_jsons_cached(action_json: Path) -> list[Path]:
    key = str(action_json.parent)
    if key not in _EPISODE_PLAYER_JSONS_CACHE:
        _EPISODE_PLAYER_JSONS_CACHE[key] = episode_player_jsons(action_json)
    return _EPISODE_PLAYER_JSONS_CACHE[key]


def project_dead_mask_for_frame(
    *,
    mesh_mod: Any,
    other_frames_by_stem: dict[str, list[dict[str, Any]]],
    ego_frame: dict[str, Any],
    ego_stem: str,
    frame_index: int,
    latent_h: int,
    latent_w: int,
    video_h: int,
    video_w: int,
    fov_x: float,
    far: float,
    pitch_sign: float,
) -> tuple[np.ndarray, int, list[str]]:
    mask = np.zeros((latent_h, latent_w), dtype=np.uint8)
    cam_pos = np.asarray(ego_frame.get("camera_position") or [ego_frame["x"], ego_frame["y"], ego_frame["z"] + 64.0], dtype=np.float32)
    yaw = float(ego_frame.get("yaw", ego_frame.get("camera_rotation", [0, 0, 0])[2]))
    pitch = float(ego_frame.get("pitch", ego_frame.get("camera_rotation", [0, 0, 0])[1]))
    projected = 0
    projected_stems: list[str] = []
    for stem, frames in other_frames_by_stem.items():
        if stem == ego_stem or frame_index >= len(frames):
            continue
        frame = frames[frame_index]
        if not finite_player(frame):
            continue
        if float(frame.get("health", 100.0)) > 0.0:
            continue
        base = np.asarray([frame["x"], frame["y"], frame["z"] + PLAYER_Z_OFFSET], dtype=np.float32)
        samples = np.asarray([
            [base[0], base[1], base[2] + 4.0],
            [base[0], base[1], base[2] + PLAYER_HEIGHT * 0.18],
            [base[0], base[1], base[2] + PLAYER_HEIGHT * 0.35],
        ], dtype=np.float32)
        proj, z_cam = mesh_mod.project_vertices(samples, cam_pos, yaw + CAMERA_YAW_OFFSET, pitch + CAMERA_PITCH_OFFSET, pitch_sign, video_w, video_h, fov_x)
        valid = (z_cam > 1.0) & (z_cam < far)
        if not np.any(valid):
            continue
        u = float(np.mean(proj[valid, 0])) * latent_w / video_w
        v = float(np.mean(proj[valid, 1]) + PLAYER_SCREEN_Y_OFFSET_PX) * latent_h / video_h
        z = float(np.mean(z_cam[valid]))
        base_pixel_radius = PLAYER_RADIUS / max(z, 1.0) / np.tan(np.radians(fov_x) / 2.0) * video_w * 0.5
        rx = max(1.0, base_pixel_radius * PLAYER_RADIUS_SCALE * latent_w / video_w * 1.6)
        ry = max(1.0, rx * 0.45)
        if u < -rx or u >= latent_w + rx or v < -ry or v >= latent_h + ry:
            continue
        stamp_ellipse(mask, u, v, rx, ry, 1)
        projected += 1
        projected_stems.append(stem)
    return mask, projected, projected_stems


def build_one(record: dict[str, Any], out_dir: Path, mesh_mod: Any, *, video_h: int, video_w: int, latent_h: int, latent_w: int, fov_x: float, far: float, pitch_sign: float) -> tuple[dict[str, Any], dict[str, Any]]:
    clip_id = str(record["clip_id"])
    raw_frames = [int(x) for x in record["map_memory_raw_frame_indices"]]
    action_json = Path(record["action_json"])
    ego_stem = str(record.get("map_memory_ego_stem") or record.get("player_stem"))
    ego_frames = read_json_cached(action_json)
    alive, health, ego_debug = ego_state(ego_frames, raw_frames)

    try:
        interaction = build_interaction_channels(
            frames=ego_frames,
            raw_frames=raw_frames,
        )
    except Exception as exc:
        raise RuntimeError(
            f"clip_id={clip_id}: failed to build interaction channels: {exc}"
        ) from exc
    interaction = validate_interaction_channels(
        interaction, clip_id=clip_id, frame_count=len(raw_frames)
    )
    interaction_summary = interaction_stats(interaction)

    fire_event = interaction["ego_fire_event"]
    reload_event = interaction["ego_reload_event"]
    weapon_switch_event = interaction["ego_weapon_switch_event"]
    throw_event = interaction["ego_throw_event"]
    weapon_slot_name = interaction["ego_weapon_slot_name"]

    other_frames_by_stem: dict[str, list[dict[str, Any]]] = {}
    for p in episode_player_jsons_cached(action_json):
        stem = p.stem
        if stem == ego_stem:
            continue
        # parse filter keeps only player files and drops unrelated jsons.
        try:
            parse_team_player(stem)
        except Exception:
            continue
        other_frames_by_stem[stem] = read_json_cached(p)

    dead_masks = np.zeros((len(raw_frames), latent_h, latent_w), dtype=np.uint8)
    dead_projected_counts: list[int] = []
    dead_stems_by_frame: list[list[str]] = []
    for i, frame_index in enumerate(raw_frames):
        if frame_index < 0 or frame_index >= len(ego_frames):
            dead_projected_counts.append(0)
            dead_stems_by_frame.append([])
            continue
        m, count, stems = project_dead_mask_for_frame(
            mesh_mod=mesh_mod,
            other_frames_by_stem=other_frames_by_stem,
            ego_frame=ego_frames[frame_index],
            ego_stem=ego_stem,
            frame_index=frame_index,
            latent_h=latent_h,
            latent_w=latent_w,
            video_h=video_h,
            video_w=video_w,
            fov_x=fov_x,
            far=far,
            pitch_sign=pitch_sign,
        )
        dead_masks[i] = m
        dead_projected_counts.append(int(count))
        dead_stems_by_frame.append(stems)

    rel = Path(sha1_text(clip_id)[:2]) / f"{clip_id}.npz"
    out_path = out_dir / rel
    out_path.parent.mkdir(parents=True, exist_ok=True)
    np.savez_compressed(
        out_path,
        ego_alive=alive.astype(np.float32),
        ego_health=health.astype(np.float32),
        opponent_dead_mask=dead_masks.astype(np.uint8),
        ego_fire_event=fire_event,
        ego_reload_event=reload_event,
        ego_weapon_switch_event=weapon_switch_event,
        ego_throw_event=throw_event,
        ego_weapon_slot_name=weapon_slot_name,
        ego_current_weapon_onehot=interaction["ego_current_weapon_onehot"],
        ego_fire_weapon_onehot=interaction["ego_fire_weapon_onehot"],
        ego_reload_weapon_onehot=interaction["ego_reload_weapon_onehot"],
        ego_switch_target_onehot=interaction["ego_switch_target_onehot"],
        ego_throw_item_onehot=interaction["ego_throw_item_onehot"],
        raw_frames=np.asarray(raw_frames, dtype=np.int32),
    )
    row = {
        "kind": "state_channels_v1_record",
        "clip_id": clip_id,
        "state_cache": str(out_path),
        "source_cache_manifest": record.get("_source_cache_manifest"),
        "latent_cache": record.get("latent_cache"),
        "map_memory_raw_frame_indices": raw_frames,
        "weapon_vocabulary": list(WEAPON_VOCABULARY),
        "weapon_vocab_size": len(WEAPON_VOCABULARY),
        "shape": {
            "ego_alive": [len(raw_frames)],
            "ego_health": [len(raw_frames)],
            "opponent_dead_mask": [
                len(raw_frames),
                latent_h,
                latent_w,
            ],
            "ego_fire_event": [len(raw_frames)],
            "ego_reload_event": [len(raw_frames)],
            "ego_weapon_switch_event": [len(raw_frames)],
            "ego_throw_event": [len(raw_frames)],
            "ego_weapon_slot_name": [len(raw_frames)],
            "ego_current_weapon_onehot": [len(raw_frames), len(WEAPON_VOCABULARY)],
            "ego_fire_weapon_onehot": [len(raw_frames), len(WEAPON_VOCABULARY)],
            "ego_reload_weapon_onehot": [len(raw_frames), len(WEAPON_VOCABULARY)],
            "ego_switch_target_onehot": [len(raw_frames), len(WEAPON_VOCABULARY)],
            "ego_throw_item_onehot": [len(raw_frames), len(WEAPON_VOCABULARY)],
        },
        "channels": [
            "ego_alive_constant_plane",
            "ego_health_norm_constant_plane",
            "opponent_dead_marker_mask",
            "ego_fire_event_constant_plane",
            "ego_reload_event_constant_plane",
            "ego_weapon_switch_event_constant_plane",
            "ego_throw_event_constant_plane",
            "ego_current_weapon_onehot",
            "ego_fire_weapon_onehot",
            "ego_reload_weapon_onehot",
            "ego_switch_target_onehot",
            "ego_throw_item_onehot",
        ],
        "metadata_arrays": [
            "ego_weapon_slot_name",
        ],
      
        "stats": {
            "ego_alive_min": float(alive.min()) if len(alive) else None,
            "ego_health_min": float(health.min()) if len(health) else None,
            "ego_dead_frames": int((alive <= 0.0).sum()),
            "opponent_dead_mask_frames": int(sum(1 for x in dead_projected_counts if x > 0)),
            "opponent_dead_mask_pixels": int(dead_masks.sum()),
            "dead_projected_counts": dead_projected_counts,
            "ego_fire_event_frames": int(fire_event.sum()),
            "ego_reload_event_frames": int(reload_event.sum()),
            "ego_weapon_switch_event_frames": int(
                weapon_switch_event.sum()
            ),
            "ego_throw_event_frames": int(throw_event.sum()),
            "interaction_event_counts": interaction_summary["event_counts"],
            "interaction_event_target_distribution": interaction_summary[
                "event_target_distribution"
            ],
            "interaction_unknown_other": interaction_summary["unknown_other"],
            "weapon_names": sorted(
                {
                    str(name)
                    for name in weapon_slot_name.tolist()
                    if str(name)
                }
            ),
        },
    }

    debug = {
        "clip_id": clip_id,
        "action_json": str(action_json),
        "ego_stem": ego_stem,
        "raw_frames": raw_frames,
        "ego_debug": ego_debug,
        "dead_stems_by_frame": dead_stems_by_frame,
        "fire_event_latent_indices": np.flatnonzero(
            fire_event
        ).tolist(),
        "weapon_switch_latent_indices": np.flatnonzero(
            weapon_switch_event
        ).tolist(),
        "reload_event_latent_indices": np.flatnonzero(reload_event).tolist(),
        "throw_event_latent_indices": np.flatnonzero(throw_event).tolist(),
        "interaction_event_target_distribution": interaction_summary[
            "event_target_distribution"
        ],
        "weapon_slot_name": weapon_slot_name.tolist(),
    }
    return row, debug


def main() -> None:
    ap = argparse.ArgumentParser(description=__doc__)
    ap.add_argument("--cache-manifest", type=Path, action="append", default=[])
    ap.add_argument("--out-dir", type=Path, default=Path("output/state_channels_v1"))
    ap.add_argument("--limit", type=int, default=None)
    ap.add_argument("--video-height", type=int, default=480)
    ap.add_argument("--video-width", type=int, default=832)
    ap.add_argument("--latent-height", type=int, default=60)
    ap.add_argument("--latent-width", type=int, default=104)
    ap.add_argument("--fov-x", type=float, default=106.26)
    ap.add_argument("--far", type=float, default=4096.0)
    ap.add_argument("--pitch-sign", type=float, default=-1.0)
    ap.add_argument("--clean", action="store_true", help="Remove existing state cache/report files under --out-dir before building.")
    ap.add_argument("--state-dir-name", default="state_cache_v1")
    ap.add_argument("--manifest-name", default="state_cache_manifest_v1.jsonl")
    ap.add_argument("--report-name", default="state_cache_build_report_v1.json")
    args = ap.parse_args()

    manifests = args.cache_manifest or DEFAULT_CACHE_MANIFESTS
    state_dir = args.out_dir / args.state_dir_name
    if args.clean and args.out_dir.exists():
        for p in [state_dir, args.out_dir / args.manifest_name, args.out_dir / args.report_name]:
            if p.is_dir():
                import shutil

                shutil.rmtree(p)
            elif p.exists():
                p.unlink()
    state_dir.mkdir(parents=True, exist_ok=True)
    mesh_mod = import_tool(ROOT / "tools" / "build_mesh_projection_v0.py", "mesh_projection_v0_state_v1")

    records: list[dict[str, Any]] = []
    for p in manifests:
        for row in iter_jsonl(p):
            row = dict(row)
            row["_source_cache_manifest"] = str(p)
            records.append(row)
    if args.limit is not None:
        records = records[: args.limit]

    out_manifest = args.out_dir / args.manifest_name
    debug_samples: list[dict[str, Any]] = []
    counts = Counter()
    event_target_totals = {
        event_name: Counter() for event_name, _, _ in EVENT_TARGET_FIELDS
    }
    unknown_other_totals = {field: Counter() for field in ONEHOT_FIELDS}
    rows: list[dict[str, Any]] = []
    for idx, record in enumerate(records):
        row, debug = build_one(record, state_dir, mesh_mod, video_h=args.video_height, video_w=args.video_width, latent_h=args.latent_height, latent_w=args.latent_width, fov_x=args.fov_x, far=args.far, pitch_sign=args.pitch_sign)
        row["state_cache"] = str(Path(row["state_cache"]).relative_to(args.out_dir))
        rows.append(row)
        st = row["stats"]
        counts["rows"] += 1
        counts["ego_dead_windows"] += int(st["ego_dead_frames"] > 0)
        counts["opponent_dead_windows"] += int(st["opponent_dead_mask_frames"] > 0)
        counts["windows_without_any_death_signal"] += int(st["ego_dead_frames"] == 0 and st["opponent_dead_mask_frames"] == 0)
        counts["opponent_dead_mask_pixels"] += int(st["opponent_dead_mask_pixels"])
        for event_name, event_field, _ in EVENT_TARGET_FIELDS:
            frame_count = int(st[f"{event_field}_frames"])
            counts[f"{event_name}_windows"] += int(frame_count > 0)
            counts[f"{event_name}_event_frames"] += frame_count
            event_target_totals[event_name].update(
                st["interaction_event_target_distribution"][event_name]
            )
        for field in ONEHOT_FIELDS:
            field_stats = st["interaction_unknown_other"][field]
            unknown_other_totals[field]["sample_count"] += int(
                field_stats["sample_count"]
            )
            unknown_other_totals[field]["unknown_count"] += int(
                field_stats["unknown_count"]
            )
            unknown_other_totals[field]["other_count"] += int(
                field_stats["other_count"]
            )
        if len(debug_samples) < 3:
            debug_samples.append(debug)
        if (idx + 1) % 100 == 0:
            print(json.dumps({"event": "progress", "done": idx + 1, "total": len(records)}, ensure_ascii=False), flush=True)

    with out_manifest.open("w", encoding="utf-8") as f:
        for row in rows:
            f.write(json.dumps(row, ensure_ascii=False) + "\n")
    by_manifest = Counter(row["source_cache_manifest"] for row in rows)
    cache_files = sorted(state_dir.glob("*/*.npz"))
    aggregate_unknown_other: dict[str, dict[str, float | int]] = {}
    for field, field_counts in unknown_other_totals.items():
        sample_count = int(field_counts["sample_count"])
        unknown_count = int(field_counts["unknown_count"])
        other_count = int(field_counts["other_count"])
        aggregate_unknown_other[field] = {
            "sample_count": sample_count,
            "unknown_count": unknown_count,
            "unknown_ratio": float(unknown_count / sample_count) if sample_count else 0.0,
            "other_count": other_count,
            "other_ratio": float(other_count / sample_count) if sample_count else 0.0,
        }
    report = {
        "kind": "state_channels_v1_build_report",
        "status": "pass",
        "out_dir": str(args.out_dir),
        "state_cache_manifest": str(out_manifest),
        "state_cache_dir": str(state_dir),
        "input_cache_manifests": [str(p) for p in manifests],
        "input_rows": len(records),
        "written_rows": len(rows),
        "state_cache_file_count": len(cache_files),
        "weapon_vocabulary": list(WEAPON_VOCABULARY),
        "weapon_vocab_size": len(WEAPON_VOCABULARY),
        "counts": dict(counts),
        "death_window_stats": {
            "ego_dead_windows": int(counts["ego_dead_windows"]),
            "opponent_dead_windows": int(counts["opponent_dead_windows"]),
            "windows_without_any_death_signal": int(counts["windows_without_any_death_signal"]),
            "opponent_dead_mask_pixels": int(counts["opponent_dead_mask_pixels"]),
        },
        "rows_by_input_manifest": dict(by_manifest),
        "mtime_utc": {
            "manifest": mtime_utc(out_manifest),
            "report": None,
            "first_cache_file": mtime_utc(cache_files[0]) if cache_files else None,
            "last_cache_file": mtime_utc(cache_files[-1]) if cache_files else None,
        },
        "storage": "compressed npz per clip: ego scalar vectors [21] plus opponent_dead_mask uint8 [21,60,104]; constant planes are expanded by trainer on demand",
        "fallbacks": {"missing_ego_health": "health=1.0/alive=1.0", "missing_player_frame": "skip dead marker for that player/frame"},
        "health_audit_samples": debug_samples,
        "interaction_window_stats": {
            "fire_windows": int(counts["fire_windows"]),
            "reload_windows": int(counts["reload_windows"]),
            "weapon_switch_windows": int(
                counts["weapon_switch_windows"]
            ),
            "throw_windows": int(counts["throw_windows"]),
            "fire_event_frames": int(
                counts["fire_event_frames"]
            ),
            "reload_event_frames": int(counts["reload_event_frames"]),
            "weapon_switch_event_frames": int(
                counts["weapon_switch_event_frames"]
            ),
            "throw_event_frames": int(counts["throw_event_frames"]),
            "event_target_distribution": {
                event_name: dict(sorted(distribution.items()))
                for event_name, distribution in event_target_totals.items()
            },
            "unknown_other": aggregate_unknown_other,
        },
    }
    report_path = args.out_dir / args.report_name
    report_path.write_text(json.dumps(report, ensure_ascii=False, indent=2), encoding="utf-8")
    report["mtime_utc"]["report"] = mtime_utc(report_path)
    report_path.write_text(json.dumps(report, ensure_ascii=False, indent=2), encoding="utf-8")
    print(json.dumps(report, ensure_ascii=False, indent=2))


if __name__ == "__main__":
    main()

...
def sample_metrics(memory: np.ndarray, teacher_depth: np.ndarray, teacher_hit: np.ndarray, teacher_semantic: np.ndarray, teacher_player: np.ndarray) -> dict[str, Any]:
    ...
    return {
        ...
        "other_player_memory_pixels": int(mem_player.sum()),
        "other_player_teacher_pixels": int(teacher_player_b.sum()),
...
def main() -> None:
    ...
    for sample in manifest.get("samples", [])[: args.max_samples]:
        match_dir = Path(sample.get("match_dir") or manifest.get("match_dir") or args.match_dir)
        episode = sample.get("raw_episode") or sample["episode"]

#!/usr/bin/env python3
"""Find first-person frames with teacher-visible non-ego players.
...
def main() -> None:
    ...
    for ego_stem in player_stems(episode_dir)[: args.ego_limit]:
        ...
        for frame_index, visibility_score in prefilter:
            ...
            ego_rows.append({
                ...
                "score": float(pixels + player_count * 1000),
            })
    ...
    out = {
        "kind": "teacher_other_player_candidates_v0",
        "match_dir": str(args.match_dir),
        "episode": args.episode,
        "policy": "Teacher RGB/depth/seg/visibility are used only to choose positive QA frames. Dense inputs must still be rendered from Map Memory.",
        "height": args.height,
        "width": args.width,

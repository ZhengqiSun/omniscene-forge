#!/usr/bin/env python3
"""Candidate1 sequential I2V continuation wrapper with periodic GT context anchors.
...
def stitch(ffmpeg: str, mp4s: list[Path], out_mp4: Path) -> dict[str, Any]:
    ...
    concat = out_mp4.with_suffix(".concat.txt")
...
def main() -> None:
    ...
    ap.add_argument("--out-root", type=Path, default=ROOT / "output/ar_anchor_exp_20260706_v0")
    ap.add_argument("--anchor-k", type=int, required=True, help="Use original GT clip image as context every K windows; window0 is always GT.")
    ...
    gen_dir = args.out_root / f"gen_true_dense_ar_anchorK{args.anchor_k}"
    out_dir = gen_dir / "out"
    ...
    def set_active_dense(manifest: Path) -> None:
        nonlocal active_samples, active_sample_ids, active_dense_manifest
    ...
    run_report = {
        "kind": "candidate1_ar_anchor_wrapper_v0",
        ...
        "context_mode": f"window0 original clip_dir/image.jpg; windowN uses original GT clip image when index % anchor_k == 0, otherwise previous generated tail_frame.png; anchor_k={args.anchor_k}",
        "windows": [],
    }
    write_json(args.out_root / f"candidate1_ar_anchorK{args.anchor_k}_run_report_v0.json", run_report)
    ...
    for idx, row in enumerate(rows):
        ...
        set_active_dense(Path(row["phase2a_dense_sequence_manifest"]))
        original_image = clip_dir / "image.jpg"
        use_gt_anchor = (idx == 0) or (idx % args.anchor_k == 0)
        context_image = original_image if use_gt_anchor else prev_tail
        if context_image is None:
            raise RuntimeError("missing previous tail for non-anchor window")
        ...
        mp4 = out_dir / f"{clip_id}_true_dense_ar_anchorK{args.anchor_k}_lf{eff_lf}_seed{args.seed}.mp4"
        ...
        rec: dict[str, Any] = {
            ...
            "context_image_kind": "gt_anchor_image" if use_gt_anchor else "generated_tail_frame",
            "anchor_k": args.anchor_k,
            "is_gt_anchor_window": bool(use_gt_anchor),
        ...
        try:
            ...
            write_json(out_dir / f"{clip_id}_true_dense_ar_anchorK{args.anchor_k}_report.json", rec)
    ...
    stitched = stitch(args.ffmpeg, mp4s, args.out_root / f"stitched/candidate1_ar_anchorK{args.anchor_k}_1min.mp4")
    run_report.update({
        "status": "complete",
        "completed_at": time.strftime("%F %T %z"),

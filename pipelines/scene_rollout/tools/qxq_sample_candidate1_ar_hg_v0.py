#!/usr/bin/env python3
"""Candidate1 sequential I2V continuation wrapper with MultiGen history guidance.
...
def main() -> None:
    ...
    ap.add_argument("--history-guidance", action=argparse.BooleanOptionalAction, default=True,
                    help="MultiGen HG: clean context for CFG-cond, noised context for CFG-uncond")
    ap.add_argument("--history-guidance-noise-std", type=float, default=0.5,
                    help="Gaussian std on normalized RGB [-1,1] for CFG-uncond context; 0 disables")
    ap.add_argument("--history-guidance-seed-offset", type=int, default=9176)
    ...
    import wan
    from wan_history_guidance_patch_v0 import install_history_guidance
    ...
    run_report = {
        ...
        "history_guidance": bool(args.history_guidance),
        "history_guidance_noise_std": float(args.history_guidance_noise_std),
        "history_guidance_seed_offset": int(args.history_guidance_seed_offset),
        "history_guidance_note": "MultiGen-style inference-only HG: CFG conditional y uses clean context image; CFG unconditional y uses Gaussian-noised context image. Noise std default 0.5 is a local assumption, not a quoted paper hyperparameter.",
        "windows": [],
    }
    ...
    for idx, row in enumerate(rows):
        ...
        context_image = prev_tail if prev_tail is not None else original_image

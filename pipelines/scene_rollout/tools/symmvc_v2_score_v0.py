...
def frame_metrics(pair: dict, variant: str, roma, det, dense_index, args, tmpd: str) -> dict:
    ...
    if not mp_a or not mp_b or not Path(mp_a).exists() or not Path(mp_b).exists():
        return {"status": "missing_video", "frames": []}
    ...
    for li, raw, pix_ab, pix_ba in frames:
        ...
        rows.append(row)
    return {"status": "ok", "videoA": mp_a, "videoB": mp_b, "frames": rows}
...
def summarize_pair(res: dict, threshold: float | None) -> dict:
    ...
    return {"covisible": stat(cov, threshold), "dynamic_player": stat(dyn, threshold), "frames_ok": sum(1 for f in res.get("frames", []) if f.get("status") == "ok"), "matched_points_total": int(sum(mcert)), "matched_points_mean": round(float(np.mean(mcert)), 3) if mcert else None, "mean_confidence": round(float(np.mean(confs)), 4) if confs else None}

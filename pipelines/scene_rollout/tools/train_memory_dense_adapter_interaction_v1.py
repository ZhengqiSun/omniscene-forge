...
import map_memory_training_data_v0 as _mmtd
from map_memory_training_data_v0 import (
    ...
    sample_from_qxq_manifest,
    sha256_file,
    split_report,
    split_samples,
    validate_qxq_release_manifest,
)
...
def active_dense_hw() -> tuple[int, int]:
    """Current dense spatial size, honoring QXQ v2 releases.

    validate_qxq_release_manifest rewrites map_memory_training_data_v0.SHAPE to
    the manifest's real dense shape, so read it as a module attribute; the
    from-import above would freeze the legacy (7,176,320) value.
    Legacy releases never touch SHAPE, so this still returns (176, 320) there.
    """
    shape = tuple(int(v) for v in _mmtd.SHAPE)
    if len(shape) != 3:
        raise ValueError(f"unexpected dense SHAPE {shape}")
    return shape[1], shape[2]


def dense_native_vae_hw(vae_stride: int) -> tuple[int, int]:
    dense_h, dense_w = active_dense_hw()
    if dense_h % vae_stride or dense_w % vae_stride:
        raise ValueError(
            f"dense hw {(dense_h, dense_w)} not divisible by --vae-stride {vae_stride}"
        )
    return dense_h // vae_stride, dense_w // vae_stride
...
def build_dense_encoder(
    ...
    *,
    ...
    encoder = MemoryDenseFrozenVAEWanTokenEncoder(
        ...
        vae=vae,
        vae_pth=vae_pth,
        native_hw=dense_native_vae_hw(args.vae_stride),
...
def build_lightweight_qxq_release(
    manifest_path: Path,
    ...
    *,
    ...
    """Lightweight index for QXQ dense v2 releases (240x416, role derived).

    The v2 loader already existed in map_memory_training_data_v0 but was never
    wired into this trainer, so v2 releases hit the legacy 176x320 gate. This
    delegates to the project's own validators instead of relaxing that gate:
    shape comes from the manifest, and selection_role follows the canonical
    other_player_memory_pixels rule.
    """
    ...
    samples: list[MapMemorySample] = [
        sample_from_qxq_manifest(manifest_path, sample)
        for sample in manifest.get("samples", [])
        if str(sample.get("sample_id", "")) in wanted
    ]
    missing = sorted(wanted - {sample.sample_id for sample in samples})
    if missing:
        raise ValueError(
            f"{manifest_path}: qxq lightweight index missing sample ids first={missing[:5]}"
        )
    return MapMemoryRelease(manifest_path, manifest, readiness, teacher_qa, samples)
...
def build_lightweight_release_from_manifest(
    ...
    *,
    ...
    if str(manifest.get("kind", "")) == QXQ_RELEASE_KIND:
        return build_lightweight_qxq_release(
            manifest_path,
            manifest,
            require_backend_id=require_backend_id,
            sample_ids=sample_ids,
        )
    if manifest.get("shape") != list((7, 176, 320)):
        raise ValueError(f"{manifest_path}: manifest shape {manifest.get('shape')} != {[7, 176, 320]}")
    ...
    for sample in samples_raw:
        ...
        if sample.get("shape") != list((7, 176, 320)):
            raise ValueError(f"{manifest_path}: {sid}: sample shape {sample.get('shape')} != {[7, 176, 320]}")
...
def read_manifest_top_level_shape(path: Path, *, max_bytes: int = 4 << 20) -> tuple[int, ...] | None:
    """Read only the top-level "shape" of a multi-GB release manifest.

    Release manifests are 1-2 GB almost entirely because of "samples", but
    "shape" sits in the header. Scan with brace depth tracking so a per-sample
    "shape" can never be mistaken for the manifest-level one.
    """
    ...
    while i < n:
        ...
        if ch == '"':
            try:
                key, end = decoder.raw_decode(head[i:])
            except ValueError:
                return None
            j = i + end
            while j < n and head[j] in " \t\r\n":
                j += 1
            if depth == 1 and key == "shape" and j < n and head[j] == ":":
                k = j + 1
                while k < n and head[k] in " \t\r\n":
                    k += 1
                try:
                    value, _ = decoder.raw_decode(head[k:])
                except ValueError:
                    return None
...
def sync_dense_shape_from_manifests(manifest_pairs: list[tuple[Path, Path]]) -> tuple[int, ...]:
    """Restore map_memory_training_data_v0.SHAPE without re-validating manifests.

    The v2 loader propagates dense shape as a side effect of
    validate_qxq_release_manifest, which the training-index-cache hit path never
    calls (releases are rebuilt from cached rows). Without this, load_dense would
    check 240x416 payloads against the stale legacy 176x320 default.
    """
    shapes: dict[tuple[int, ...], list[str]] = {}
    for map_manifest, _cache_manifest in manifest_pairs:
        shape = read_manifest_top_level_shape(map_manifest.resolve())
        if shape is None:
            raise ValueError(f"{map_manifest}: cannot read top-level dense shape from manifest header")
        shapes.setdefault(shape, []).append(str(map_manifest))
    if len(shapes) > 1:
        detail = "; ".join(f"{list(k)}: {len(v)} release(s)" for k, v in shapes.items())
        raise ValueError(
            "releases mix dense spatial shapes, which load_dense cannot support "
            f"because it validates against one process-wide shape ({detail})"
        )
...
def load_training_index_cache(
    ...
    *,
    ...
    dense_shape = sync_dense_shape_from_manifests(manifest_pairs)
    log_loader_stage(
        args,
        "training_index_cache_releases_restored",
...
def load_and_validate_training_inputs(
    ...
    if args.dense_resize_mode != "bilinear":
        dense_native_hw = (176 // args.vae_stride // args.patch_size_hw, 320 // args.vae_stride // args.patch_size_hw)
        if wan_token_hw != dense_native_hw:
            raise ValueError(
                f"cache Wan grid {wan_token_hw} differs from native dense grid {dense_native_hw}; "
                "use --dense-resize-mode bilinear to train 480p cache with v43 dense maps"
            )
    log_loader_stage(
        args,
...
def check_noop(args: argparse.Namespace) -> dict[str, Any]:
    ...
    latent_h = 176 // args.vae_stride
    latent_w = 320 // args.vae_stride
...
def build_parser() -> argparse.ArgumentParser:
    ...
    ap.add_argument(
        ...
        default="bilinear",
        help="Resize encoded 176x320 dense tokens to the cache Wan token grid, or require the native 11x20 grid.",
    )

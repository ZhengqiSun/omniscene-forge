...
DROPLOG VARIANT (qxq, added 2026-07-18 by Claude-Worker-0 data-audit agent):
This file is a byte-for-byte copy of the zhengqi production renderer that rendered
the W3 zero-visibility dataset (source sha256 fd368b511ecfdad2da3459741637485e299a48e36608c161fc46264023f6a510,
/mnt/workspace/zhengqi/multiview-map-v0/tools/build_mesh_dense_condition_v0.py, 1099L),
plus a single additive diagnostic feature: `project_memory_players` now also emits a
`dropped_players` list in its meta dict, recording every OTHER player that was
considered but culled at each decision gate (frame-out-of-range, non-finite pose,
dead, z-gate, off-screen, below min_visible_pixels / occlusion), with the fields
available at that gate (player_key, team_id, is_enemy, z_cam, projected_pixels,
drop_reason). This is meta-only: NO rendered output array is changed. Every other
player in a frame is now accounted for as exactly one of memory_projected_players
(rendered) or dropped_players (culled). Kept as a separate filename to preserve the
qxq committed build_mesh_dense_condition_v0.py (which diverges from production by 76
changed lines) and to make provenance vs the W3 render explicit.
"""
...
# Canonical player capsule calibration used by CPU/GPU dense exports.
PLAYER_RADIUS = 14.0
PLAYER_HEIGHT = 40.0
PLAYER_OCCLUSION_TOLERANCE = 40.0
...
def load_renderer_cache(match_dir: Path, tools_dir: Path | None = None, bsp_faces_npz: Path | None = None) -> dict[str, Any]:
    ...
    try:
        ...
        cache.update({
            "obj_path": None,
            "vertices": cache["bsp_vertices"],
            "faces": cache["bsp_faces"],
            "world_obj_available": False,
        })
    return cache
...
def describe_geometry_backend(mesh_backend: str, cache: dict[str, Any]) -> dict[str, Any]:
    ...
    if mesh_backend in {"bsp_faces_gpu", "bsp_faces_cpu"}:
        ...
        component_counts = dict(cache.get("bsp_component_counts", {}))
        if cache.get("world_obj_available"):
...
def project_memory_players(
    ...
    player_screen_y_offset_px: float = PLAYER_SCREEN_Y_OFFSET_PX,
    player_radius_scale: float = PLAYER_RADIUS_SCALE,
) -> tuple[np.ndarray, dict[str, Any]]:
    ...
    def _record_drop(stem, team_id, player_idx, reason, z_cam=None,
                     projected_pixels=None, raw_pixels=None, extra=None):
        # meta-only diagnostic record; never touches rendered arrays
        rec = {
            "player_key": stem,
            "team_id": (None if team_id is None else int(team_id)),
            "player_index": (None if player_idx is None else int(player_idx)),
            "is_enemy": (None if team_id is None else bool(team_id != ego_team)),
            "z_cam": (None if z_cam is None else float(z_cam)),
            "projected_pixels": (None if projected_pixels is None else int(projected_pixels)),
            "raw_pixels": (None if raw_pixels is None else int(raw_pixels)),
            "drop_reason": reason,
        }
        if extra:
            rec.update(extra)
        dropped_players.append(rec)
    ...
    for stem, path in players_iter:
        ...
        if episode_memory is None:
            ...
            if frame_index >= len(frames):
                _record_drop(stem, team_id, player_idx, "frame_index_out_of_range")
                continue
            frame = frames[frame_index]
        else:
            frame = frame_from_episode_memory(episode_memory, stem, frame_index)
        if not np.isfinite(frame["x"]):
            _record_drop(stem, team_id, player_idx, "nonfinite_position")
            continue
        if float(frame.get("health", 100)) <= 0:
            _record_drop(stem, team_id, player_idx, "dead_health_le_0",
                         extra={"health": float(frame.get("health", 100))})
            continue
        ...
        if not np.any(valid):
            _record_drop(stem, team_id, player_idx, "z_gate_no_valid_vertex",
                         extra={"z_cam_minmax": [float(np.min(z_cam)), float(np.max(z_cam))]})
            continue
        u = float(np.mean(proj[valid, 0]))
        v_top = float(np.min(proj[valid, 1]))
        v_bot = float(np.max(proj[valid, 1]))
        z = float(np.mean(z_cam[valid]))
        base_pixel_radius = player_radius / max(z, 1.0) / np.tan(np.radians(fov_x) / 2.0) * width * 0.5
        pixel_radius = max(1.0, base_pixel_radius * player_radius_scale)
        y_mid = (v_top + v_bot) * 0.5 + player_screen_y_offset_px
        ...
        if u < -pixel_radius or u >= width + pixel_radius or y_mid < -capsule_h or y_mid >= height + capsule_h:
            _record_drop(stem, team_id, player_idx, "offscreen_bbox", z_cam=z,
                         extra={"center_uv": [u, y_mid], "pixel_radius": pixel_radius,
        ...
        if int(visible_pixels.sum()) < min_visible_pixels:
            _raw_px = int(raw_player_mask.sum())
            _vis_px = int(visible_pixels.sum())
            _record_drop(stem, team_id, player_idx, "below_min_visible_pixels", z_cam=z,
                         projected_pixels=_vis_px, raw_pixels=_raw_px,
                         extra={"occluded_pixels": _raw_px - _vis_px,
                                "min_visible_pixels": int(min_visible_pixels),
                                "center_uv": [u, y_mid], "pixel_radius": pixel_radius})
            continue
        ...
        players_meta.append({
            ...
            "player_screen_y_offset_px": player_screen_y_offset_px,
            "player_radius_scale": player_radius_scale,
            "capsule_height_px": capsule_h,
    ...
    return (
        ...
        {"memory_projected_players": players_meta, "dropped_players": dropped_players},
    )
...
def render_memory_dense_condition(
    ...
    meta = {
        ...
        "obj_path": str(cache.get("obj_path")),
        "world_obj_available": bool(cache.get("world_obj_available", cache.get("obj_path") is not None)),
        "episode": episode,
        "ego_stem": ego_stem,
...
def main() -> None:
    ...
    ap.add_argument("--player-screen-y-offset-px", type=float, default=PLAYER_SCREEN_Y_OFFSET_PX)
    ap.add_argument("--player-radius-scale", type=float, default=PLAYER_RADIUS_SCALE)

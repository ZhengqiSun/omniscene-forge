...
def main() -> None:
    ...
    ap.add_argument("--static-memory-json", type=Path, default=None)
    args = ap.parse_args()
    ...
    geometry_backend_ids = sorted({s.get("geometry_backend_id") for s in samples if s.get("geometry_backend_id")})
    backend_signatures = sorted({s.get("backend_signature") for s in samples if s.get("backend_signature")})
    backend_features = sorted({feature for s in samples for feature in (s.get("backend_features") or [])})
    manifest = {
        ...
        "static_memory_json": str(args.static_memory_json) if args.static_memory_json else None,
        "input_provenance": {
            "candidate_json": file_fingerprint(args.candidate_json),
            "episode_memory_meta": file_fingerprint(args.episode_memory_dir / "episode_memory_meta_v0.json") if args.episode_memory_dir else None,
            "episode_memory_npz": file_fingerprint(args.episode_memory_dir / "episode_memory_v0.npz") if args.episode_memory_dir else None,
            "bsp_faces_npz": file_fingerprint(args.bsp_faces_npz),
            "static_memory_json": file_fingerprint(args.static_memory_json),
            "match_navmesh": file_fingerprint(args.match_dir / "navmesh.json"),
            "match_static_props": file_fingerprint(args.match_dir / "static_props.json"),
            "match_mesh_manifest": file_fingerprint(args.match_dir / "mesh_manifest.json"),
        },
        "mesh_backend": mesh_backend,
        "geometry_backend": args.geometry_backend,
        ...
        "backend_policy": "All samples in a training manifest should share one geometry_backend_id unless the readiness checker is run with --allow-mixed-backend.",

...
OBJ/GPU first-person depth, nav-place semantic fallback, replay-state capsules,
alive ego filtering, teacher-only QA, and strict manifest gates. It is not the
final BSP/collision renderer, but it is the reproducible map-side pipeline that
should not rely on hand-run shell fragments. Geometry backend selection is
centralized here so BSP faces exports can use the same QA gates once the dense
renderer/exporter exposes that backend.
"""
...
def main() -> None:
    ...
    ap.add_argument("--geometry-backend", type=normalize_geometry_backend, default="obj_gpu", help="Static geometry backend: obj_gpu/obj/gpu for the current OBJ CUDA path, or bsp_faces for BSP visual faces.")
    ap.add_argument("--bsp-faces-npz", type=Path, default=DEFAULT_BSP_FACES_NPZ, help="BSP visual faces npz used by --geometry-backend bsp_faces.")
    ...
    release = {
        ...
        "known_limits": [
            "bsp_faces uses visual BSP faces only when selected; final BSP/collision ray cast is still separate work.",

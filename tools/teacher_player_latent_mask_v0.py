...
# v2 dense contract (416x240 -> 30x52 native, no interpolation): the dense grid is
# owned by map_memory_training_data_v0; read it at call time via default_dense_hw().
DEFAULT_LATENT_HW = (60, 104)
DEFAULT_DATASET_ROOT = ASSET_ROOT / "csgo-datasets"
...
def import_tool(path: Path, name: str):
    ...
    # Register BEFORE exec_module: @dataclass resolves field types through
    # sys.modules[cls.__module__], which is unset for file-path imports, so
    # executing first raises AttributeError('NoneType' has no '__dict__') on any
    # module defining a dataclass -- exactly what this fallback exists to avoid.
...
_MAP_MEMORY_DATA_MODULE: Any = None


def _map_memory_data():
    """Resolve the loader module that owns the dense-grid contract.

    Lazy and cached so this module still works when it is loaded by file path
    (importlib) from a script whose sys.path does not contain tools/. There is no
    import cycle: map_memory_training_data_v0 imports only stdlib and numpy.
    """
    global _MAP_MEMORY_DATA_MODULE
    if _MAP_MEMORY_DATA_MODULE is None:
        name = "map_memory_training_data_v0"
        module = sys.modules.get(name)
        if module is None:
            try:
                import map_memory_training_data_v0 as module  # type: ignore[no-redef]
            except ImportError:
                module = import_tool(Path(__file__).resolve().parent / f"{name}.py", name)
        _MAP_MEMORY_DATA_MODULE = module
    return _MAP_MEMORY_DATA_MODULE


def default_dense_hw() -> tuple[int, int]:
    """Current dense condition grid (H, W), read from the loader at call time.

    Defaults to the v2 dense contract (240x416). Callers that need the legacy
    pre-v2 grid switch it once via map_memory_training_data_v0.set_dense_hw().
    """
    height, width = _map_memory_data().dense_hw()
...
def teacher_player_mask_latent(
    ...
    The mask is first built on the current Memory dense grid (``default_dense_hw()``,
    v2 dense contract = 240x416) with the same full-frame teacher QA crop/FOV path,
    then area-pooled to ``latent_hw`` in a single step. Pooling through an
    intermediate grid is avoided on purpose: INTER_AREA followed by ``> 0`` only
    ever dilates a boolean mask.
    Use ``latent_hw=(60,104)`` for Wan latent cells or ``(30,52)`` for token
    grid region metrics.
    """
    dense_mask = teacher_player_mask_dense(sample, dense_hw=default_dense_hw())
...
def main() -> None:
    ...
    ap.add_argument(
        ...
        default=None,
        metavar=("H", "W"),
        help="Override the dense condition grid H W (default: v2 dense contract 240 416).",
    )
    ...
    args = ap.parse_args()
    if args.dense_hw:
        _map_memory_data().set_dense_hw(int(args.dense_hw[0]), int(args.dense_hw[1]))
    ...
    info = {
        ...
        "dense_hw": list(default_dense_hw()),

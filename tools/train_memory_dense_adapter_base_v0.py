...
def assert_only_memory_dense_trainable(model: torch.nn.Module, encoder: torch.nn.Module) -> dict[str, Any]:
    bad_model = [
        name
    ...
    frozen_lora = [name for name, param in lora_params if not param.requires_grad]
    if frozen_lora:
        raise RuntimeError(f"LoRA parameters are frozen: {frozen_lora[:20]}")
    ...
    return {
        ...
        "lora_trainable_param_count": int(sum(param.numel() for _, param in lora_params)),
        "adapter_trainable_tensor_count": len(adapter_params),
        "encoder_trainable_tensor_count": len(encoder_params),
        "lora_trainable_tensor_count": len(lora_params),
...
def trainable_state_dict(model: torch.nn.Module, encoder: torch.nn.Module) -> dict[str, Any]:
    ...
    torch.save(
        {
            ...
            "step": int(step),
            "state": trainable_state_dict(model, encoder),
...
def build_adapter_config(args: argparse.Namespace) -> MemoryDenseAdapterConfig:
    return MemoryDenseAdapterConfig(
        ...
        highfreq_branch=bool(args.highfreq_branch),
        highfreq_kind=str(args.highfreq_kind),
        highfreq_hidden_dim=int(args.highfreq_hidden_dim),
        highfreq_stride=int(args.highfreq_stride),
    )
...
def preflight_train(args: argparse.Namespace) -> dict[str, Any]:
    ...
    for idx, record in enumerate(records[:max_records]):
        ...
        del x0, cond, text_context, cam_chunk, dense_tokens
...
def train(args: argparse.Namespace) -> dict[str, Any]:
    ...
    try:
        ...
        if args.use_base_model:
            ...
            boundary_sigma = float(base_cfg.boundary)  # 0.947; sigma == t / num_train_timesteps (=1000)
            ...
            # base is non-causal full-attention: train on the FULL latent sequence per step (one chunk).
        ...
        encoder = build_dense_encoder(args, config, device=device, dtype=dtype)
        trainable_audit = assert_only_memory_dense_trainable(model, encoder)
        ...
        adapter_named_params = memory_dense_adapter_parameters(model)
        lora_named_params = lora_parameters(model)
        encoder_named_params = list(encoder.named_parameters())
        params = [p for _, p in adapter_named_params] + [p for _, p in lora_named_params] + [p for _, p in encoder_named_params]
        if not params:
        ...
        optimizer_groups: list[dict[str, Any]] = [
            {"params": [p for _, p in adapter_named_params] + [p for _, p in encoder_named_params], "lr": args.lr, "weight_decay": args.weight_decay}
        ]
        ...
        if rank == 0:
            ...
            print(json.dumps({"event": "train_start", "records": len(records), "world": world, "out_dir": str(args.out_dir)}, ensure_ascii=False), flush=True)
        ...
        for step in range(start_step + 1, args.max_steps + 1):
            ...
            for micro_step in range(args.grad_accum):
                ...
                with sync_context:
                    ...
                    if args.use_base_model:
                        # band-restricted to the active expert's noise band (anti-starvation by construction)
                    ...
                    with torch.amp.autocast("cuda", dtype=torch.bfloat16, enabled=device.type == "cuda" and dtype == torch.bfloat16):
                        if args.use_base_model:
                            # base WanModel.forward(x,t,context,seq_len,y,dit_cond_dict) — NO kv-cache/current_start (non-causal full-seq)
                            pred = train_model(
                                x=[xt],
                                t=timestep,
                        ...
                        if args.region_loss_weight > 0.0:
                            ...
                            if region_loss is not None:
                                ...
                                weighted_region_loss = args.region_loss_weight * chunk_region_loss_weight * region_loss
                        ...
                        if apply_negative_contrast:
                            if "blank" in args.negative_contrast_modes:
                                blank_pred = train_model(
                                    ...
                                    dit_cond_dict={
                                        "c2ws_plucker_emb": cam_chunk.chunk(1, dim=0),
                                        COND_KEY: torch.zeros_like(dense_tokens),
                                    },
                                    kv_cache=None,
                                    crossattn_cache=None,
                                    current_start=current_start,
                ...
                del x0, cond, x0_chunk, cond_chunk, cam_chunk, dense_tokens, noise, timestep, xt, target, pred, loss, region_loss, min_snr_weight
            grad_norm = None
            ...
            if rank == 0 and (step == 1 or step % args.log_every == 0):
                payload = {"event": "train_step", "step": step, "loss": loss_value, "chunk_role_counts": step_role_counts}
                if args.region_loss_weight > 0.0:
            ...
            if rank == 0 and (step % args.save_every == 0 or step == args.max_steps):
                ...
                save_checkpoint(ckpt, model=train_model, encoder=train_encoder, optimizer=optimizer, step=step, config=run_config)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    ap.add_argument("mode", choices=["audit-data", "prepare-index", "check-noop", "preflight-train", "train"])
    ap.add_argument(
        "--map-manifest",
        type=Path,
        default=DEFAULT_MANIFEST,
        help=(
            "Dense release manifest. The built-in default is the v1 (pre-v2) h176 release; "
            "train or audit it with the matching --dense-hw, and pass a v2 release path for "
            "the default --dense-hw 240 416."
        ),
    )
    ...
    ap.add_argument(
        "--highfreq-branch",
        action="store_true",
        help="Enable VAE-free zero-init high-frequency cond branch (single-variable delta vs W2; default OFF = byte-identical).",
    )
    ap.add_argument("--highfreq-kind", choices=["laplacian", "rawconv"], default="laplacian")
    ap.add_argument("--highfreq-hidden-dim", type=int, default=32)
    ap.add_argument("--highfreq-stride", type=int, default=16)
